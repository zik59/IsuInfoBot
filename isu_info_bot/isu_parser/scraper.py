...
def get_groups(session: Session, url: str, headers: dict) -> list:
    tree = make_tree(session, url, headers)
    groups = tree.xpath('//a[@class="toggle-vis btn btn-default black"]/text()')
    return groups


def get_students_in_group(session: Session, url: str, headers:dict) -> list:
    tree = make_tree(session, url, headers)
...
def get_all_students(session: Session, headers:dict, groups: list) -> dict:
    all_students = {}
    ...
    for group in groups:
        ...
        students_in_group = get_students_in_group(session, url, headers)
...
def main():
    with start_session() as sess:
        ...
        all_groups = get_groups(sess, groups_url, headers)
